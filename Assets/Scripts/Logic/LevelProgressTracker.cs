...
public class LevelProgressTracker : MonoBehaviour
{
    ...
    public void Initialize(float startZ, float targetDistance)
    {
        _startZ = startZ;
    ...
    private void Update()
    {
        if (!_initialized || carTransform == null || _isHidden)
            return;
    ...
    {
        float traveled = Mathf.Clamp(carTransform.position.z - _startZ, 0f, _targetDistance);
        float normalizedProgress = traveled / _targetDistance;

        if (distanceFillImage != null)
            distanceFillImage.fillAmount = normalizedProgress;

        if (distanceText != null)
            distanceText.text = $"{Mathf.FloorToInt(traveled)} m";
