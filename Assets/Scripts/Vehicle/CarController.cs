...
public class CarController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_gameManager == null || _gameManager.CurrentState != GameState.Playing || !_canMove)
            return;

        Vector3 position = transform.position;

        position.z += _moveSpeed * Time.deltaTime;
        position.x = Mathf.SmoothDamp(position.x, targetX + _hitOffsetX, ref _xVelocity, xSmoothTime);
        position.y = _fixedY;
